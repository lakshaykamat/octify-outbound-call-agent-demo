...
function KbHeader({ updatedAt }: { updatedAt?: string }) {
  return (
    <PageHeader
      eyebrow="Knowledge"
      title="Knowledge base"
      description="Source material the agent draws on for every call. Edits go live on the next dial."
      actions={updatedAt ? (
        <span className="text-xs tabular-nums text-muted-foreground">
          Updated {new Date(updatedAt).toLocaleString("en-US", { month: "short", day: "numeric", hour: "numeric", minute: "2-digit" })}
        </span>
      ) : null}
    />
  );
...
export default function KnowledgeBasePage() {
  ...
  return (
    <>
      <KbHeader updatedAt={data.updatedAt} />
